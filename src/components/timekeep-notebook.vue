    ...
    <main>
      <ul>
        ...
        <li v-for="note in notes" :key="note.id">
          <p>{{note.date.isToday() ? "Today" : `${note.date.dayOfWeekString} w${note.date.week} ${note.date.year}-${note.date.month.toString().padStart(2, "0")}-${note.date.day.toString().padStart(2, "0")}`}}</p>
