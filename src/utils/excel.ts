...
function convertTime(milliseconds: number, options: ExcelOptions): number {
  if (options.format === ExcelTimeFormat.Full) return Math.round(milliseconds / 1000) / 86400;

  if (options.format === ExcelTimeFormat.Decimal) return Math.round((milliseconds / 1000 / 60 / 60) * 100) / 100;
...
function addWeek(workbook, year, week, timekeeps, options) {
  ...
  for (const timekeep of timekeeps) {
    ...
    for (const total of totals) row.push({ type: "time", value: convertTime(total, options) });
  ...
  for (const [i, columns] of rows.entries()) {
    ...
    for (const [j, column] of columns.entries()) {
      ...
      cell.style("horizontalAlignment", "center");
      cell.style("verticalAlignment", "center");
      if (column.type === "time" && options.format === "hh:mm:ss") cell.style("numberFormat", "hh:mm:ss");
      if (column.formula) cell.formula(column.formula);
    }
  }
  ...
  for (const [i, label] of columnLabels.entries()) {
    ...
    if (options.format === "hh:mm:ss") cell.style("numberFormat", "hh:mm:ss");
...
export async function exportToExcel(timekeeps, options) {
  ...
  for (const year of years) {
    ...
    for (const week of weeks) addWeek(workbook, year, week, timekeeps, options);
  }

  // Remove the default sheet
  if (workbook.sheets().length > 1) workbook.deleteSheet(0);
